...
// 🆕 ตัดสต๊อกสินค้าลง qty ชิ้น (เทียบเท่า deductProductStock_() เดิม) — ถ้าสินค้านั้นไม่ได้ตั้งค่าจำนวนสต๊อกไว้ (ช่องว่าง) จะไม่ทำอะไร (ถือว่าไม่จำกัด)
// ถ้าหาไม่เจอสินค้าที่ชื่อตรงกัน (เช่น เป็นรายการ "ค่าสนาม" ไม่ใช่สินค้าจริง) ก็จะไม่ทำอะไรเช่นกัน ไม่ทำให้การขายล้มเหลว
async function deductProductStock(productName, qty) {
  try {
    if (!productName) return;
    const data = await getValues('Products');
    const cleanName = String(productName).trim();
    for (let i = 1; i < data.length; i++) {
      if (String(data[i][0]).trim() === cleanName) {
        const currentStock = data[i][2];
        if (currentStock === '' || currentStock === undefined || isNaN(Number(currentStock))) return; // ไม่จำกัดสต๊อก ไม่ต้องตัด
        const newStock = Math.max(0, Number(currentStock) - qty);
        await updateCell('Products', i + 1, 3, newStock);
        return;
      }
    }
  } catch (e) { /* ไม่ทำให้การขายล้มเหลว ถ้าตัดสต๊อกไม่สำเร็จก็ปล่อยผ่านไป */ }
}

module.exports = { getValues, batchGetValues, appendRow, updateCell, updateRow, columnToLetter, deductProductStock };
