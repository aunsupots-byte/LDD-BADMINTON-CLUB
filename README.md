...
├── public/
│   ├── index.html            ← หน้าลงทะเบียน + ดูรายชื่อวันนี้ (สาธารณะ ไม่ต้องล็อกอิน)
│   └── admin.html             ← หน้าแอดมิน แยกออกมาคนละหน้า (ต้องล็อกอินก่อนถึงจะเห็นข้อมูล)
├── package.json
...
### Phase 5: ทดสอบ
1. เปิด URL ที่ Render ให้มา — ตอนนี้จะเห็น**เฉพาะหน้าลงทะเบียน** (ไม่มีส่วนแอดมินปนอยู่แล้ว)
2. ลองลงทะเบียนด้วยเบอร์โทร/ชื่อเล่นทดสอบ → เช็คในชีต Registrations ว่ามีแถวใหม่ขึ้นจริง
3. กด "เข้าสู่ระบบแอดมิน" มุมขวาบน (จะพาไปหน้า `/admin.html` แยกต่างหาก) → ใส่รหัสผ่านที่ตั้งไว้ใน `ADMIN_PASSWORD` → ควรเข้าแผงแอดมินได้
...
8. ลองปิดเบราว์เซอร์แล้วเปิด `/admin.html` ใหม่ภายใน 12 ชั่วโมง → ควรเข้าแผงแอดมินได้เลยโดยไม่ต้องใส่รหัสผ่านซ้ำ (เพราะตั๋วล็อกอินยังไม่หมดอายุ)
